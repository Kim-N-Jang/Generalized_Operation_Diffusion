...
class TSPGraphDataset(torch.utils.data.Dataset):
  def __init__(self, data_file, sparse_factor=-1):
    self.data_file = data_file
    self.sparse_factor = sparse_factor
    data = np.oad(data_file , allow_pickle=True)
    self.time = data[:,0].astype(np.float32)
    self.machine = data[:,1].astype(np.int64)
    self.adj = data[:2].astype(np.float32)
    print(f'Loaded "{data_file}" with {len(self.file_lines)} lines')
  ...
  def __getitem__(self, idx):
    if self.sparse_factor <= 0:
      ...
      return (
          torch.LongTensor(np.array([idx], dtype=np.int64)),
          torch.from_numpy(time).float(),
          torch.from_numpy(adj).float(),
          torch.from_numpy(machine).float(),
      )
